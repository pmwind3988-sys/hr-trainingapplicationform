...
// ── Auth Context ──────────────────────────────────────────────────────────────
export const FormAuthContext = createContext({ userEmail: null, authState: "checking", onLogin: () => {}, onLogout: () => {} });
...
const getStoredDecision   = () => { try { return localStorage.getItem(STORAGE_KEY); } catch { return null; } };
const setStoredDecision   = v  => { try { localStorage.setItem(STORAGE_KEY, v); } catch {} };
const clearStoredDecision = () => { try { localStorage.removeItem(STORAGE_KEY); } catch {} };

// ── Banners — exported so FormPage can render them directly ───────────────────
export function LoggedInBanner({ userEmail, onLogout }) {
  ...
  return (
    <div style={{
      ...
      background: C.purplePale, borderBottom: `1px solid ${C.purpleMid}`,
      padding: "7px 32px", fontSize: 13,
      boxShadow: "0 2px 6px rgba(91,33,182,0.07)",
    }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div>
          <span style={{ fontWeight: 500, color: C.textPrimary }}>{userEmail}</span>
          <span style={{ fontSize: 11, color: C.purple, marginLeft: 8 }}>
            ✓ Signed in — submission linked to your account
          </span>
        </div>
      </div>
      <button onClick={onLogout} style={{
        background: "none", border: `1px solid ${C.purpleMid}`, borderRadius: 6,
        padding: "4px 12px", fontSize: 11, color: C.purple, cursor: "pointer",
...
export function GuestBanner({ onLogin }) {
  return (
    <div style={{
      position: "sticky", top: 56, zIndex: 40,
      display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12,
      background: C.offWhite, borderBottom: `1px solid ${C.border}`,
      padding: "7px 32px", fontSize: 12, flexWrap: "wrap",
      boxShadow: "0 2px 6px rgba(91,33,182,0.07)",
    }}>
      <span style={{ color: C.textSecond }}>
        👤 You're filling this form as a <strong>guest</strong>. Sign in to track your submissions.
      </span>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        }}>Sign in to track your submission</h2>
        ...
        <button onClick={() => remember ? onSkipRemember() : onSkip()} style={{
...
export default function FormAuthWrapper({ children, formTitle = "HR Form" }) {
  ...
  const [promptState, setPromptState] = useState("checking");

  useEffect(() => {
    if (inProgress !== InteractionStatus.None) return;
    if (isAuthenticated) { setPromptState("loggedin"); return; }
    const stored = getStoredDecision();
    setPromptState(stored === "guest" ? "guest" : "prompt");
  }, [isAuthenticated, inProgress]);

  const handleLogin        = () => instance.loginRedirect({ ...loginRequest, prompt: "select_account" });
  const handleLogout       = () => { clearStoredDecision(); instance.logoutRedirect({ postLogoutRedirectUri: window.location.href }); };
  const handleSkip         = () => setPromptState("guest");
  const handleSkipRemember = () => { setStoredDecision("guest"); setPromptState("guest"); };
  ...
  return (
    <>
      ...
      <FormAuthContext.Provider value={{ userEmail, authState: promptState, onLogin: handleLogin, onLogout: handleLogout }}>
        {children}
