...
// ── Tenant guard ──────────────────────────────────────────────────────────────
// Only users from this tenant are allowed
...
function getTenantFromAccount(account) {
  // MSAL stores the tenant ID in the account's tenantId field
  return account?.tenantId || account?.idTokenClaims?.tid || null;
}

function isAllowedTenant(account) {
  const tid = getTenantFromAccount(account);
  return tid === ALLOWED_TENANT_ID;
}
    ...
    <div style={{ position: "relative" }} ref={ref}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 13, color: "#1a3c5e", fontWeight: 500 }}>{userEmail}</div>
            <div style={{ fontSize: 11, color: "#5a8ab0", marginTop: 1 }}>
              {alreadyDone
                ? `Already actioned — Layer ${layer}`
                : `Layer ${layer} of ${total} approver`
              }
            </div>
          </div>
        </div>

        {/* Right: layer badge + chevron */}
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 11, background: "#b3d9f7", color: "#0c447c", padding: "3px 10px", borderRadius: 20, fontWeight: 500, whiteSpace: "nowrap" }}>
            Layer {layer} / {total}
          </span>
          <span style={{ fontSize: 10, color: "#5a8ab0", transition: "transform 0.2s", display: "inline-block", transform: open ? "rotate(180deg)" : "rotate(0deg)" }}>▼</span>
        </div>
      </div>
      ...
      {open && (
        <div style={{
          ...
          {/* Account info header */}
          <div style={{ padding: "12px 16px", borderBottom: "1px solid #f0f0f0", background: "#f7f8fa" }}>
            <div style={{ fontSize: 11, color: "#999", marginBottom: 2 }}>Signed in as</div>
            <div style={{ fontSize: 13, color: "#333", fontWeight: 500, wordBreak: "break-all" }}>{userEmail}</div>
          </div>
          ...
          <button
            ...
            style={{
              ...
              textAlign: "left", cursor: "pointer", fontSize: 13, color: "#444",
              display: "flex", alignItems: "center", gap: 10,
            }}
            onMouseEnter={e => e.currentTarget.style.background = "#f7f8fa"}
            onMouseLeave={e => e.currentTarget.style.background = "none"}
          >
            <span style={{ fontSize: 16 }}>🔄</span> Switch account
          </button>
          ...
          <button
            ...
            style={{
              ...
              textAlign: "left", cursor: "pointer", fontSize: 13, color: "#a93226",
              display: "flex", alignItems: "center", gap: 10,
            }}
...
function PageSkeleton({ userEmail }) {
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "32px 20px" }}>
      ...
      <div style={{ background: "#e8f4fd", border: "1px solid #b3d9f7", borderRadius: 10, padding: "12px 16px", marginBottom: 28, display: "flex", alignItems: "center", gap: 10 }}>
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f7f8fa", padding: 20 }}>
      <div style={{ background: "#fff", borderRadius: 14, padding: "44px 40px", textAlign: "center", maxWidth: 460, width: "100%", boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
        ...
        <h2 style={{ color: "#a93226", marginBottom: 10, fontSize: 20, fontWeight: 500 }}>Access Restricted</h2>
        <p style={{ color: "#666", lineHeight: 1.6, marginBottom: 20 }}>
          This approval portal is only accessible to users within the organisation.
          The account <strong>{userEmail}</strong> is not part of the authorised tenant.
        </p>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <button
            onClick={onSwitch}
            style={{ padding: "11px 20px", borderRadius: 8, border: "1px solid #b3d9f7", background: "#e8f4fd", color: "#1a6fa8", fontSize: 14, fontWeight: 500, cursor: "pointer" }}
          >
            🔄 Sign in with a different account
          </button>
          <button
            onClick={onLogout}
            style={{ padding: "11px 20px", borderRadius: 8, border: "1px solid #f5b7b1", background: "#fff", color: "#a93226", fontSize: 14, cursor: "pointer" }}
          >
            🚪 Sign out
          </button>
        </div>
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }}>
      <div style={{ background: "#fff", borderRadius: 14, padding: "32px 28px", maxWidth: 460, width: "100%", boxShadow: "0 12px 48px rgba(0,0,0,0.2)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
          <div style={{ width: 44, height: 44, borderRadius: "50%", flexShrink: 0, background: isReject ? "#fdecea" : "#e6f4ea", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>
            {isReject ? "✕" : "✓"}
          </div>
          <div>
        ...
        {isReject && (
          <div style={{ marginBottom: 20 }}>
            ...
            <textarea
              ...
              rows={3}
              style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: `1px solid ${reason.trim() ? "#e0e0e0" : "#f5b7b1"}`, fontSize: 14, color: "#333", resize: "vertical", fontFamily: "inherit", boxSizing: "border-box", outline: "none" }}
            />
          </div>
        )}
        {!isReject && (
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          <button onClick={onCancel} disabled={loading} style={{ padding: "10px 20px", borderRadius: 8, border: "1px solid #e0e0e0", background: "#fff", color: "#555", fontSize: 14, cursor: "pointer" }}>
            Cancel
          </button>
          <button
            onClick={() => onConfirm(reason)}
            disabled={loading || (isReject && !reason.trim())}
            style={{ padding: "10px 22px", borderRadius: 8, border: "none", background: isReject ? (reason.trim() ? "#a93226" : "#e0e0e0") : "#1a6fa8", color: (isReject && !reason.trim()) ? "#aaa" : "#fff", fontSize: 14, fontWeight: 500, cursor: (loading || (isReject && !reason.trim())) ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: 8 }}
          >
            {loading && <div style={{ width: 14, height: 14, border: "2px solid rgba(255,255,255,0.4)", borderTop: "2px solid #fff", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />}
...
function Field({ label, value, full = false }) {
  return (
    <div style={{ gridColumn: full ? "1 / -1" : undefined, marginBottom: 4 }}>
      <div style={{ fontSize: 11, color: "#999", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.5px", fontWeight: 500 }}>{label}</div>
      <div style={{ padding: "9px 13px", background: "#f7f8fa", borderRadius: 6, border: "1px solid #eaeaea", fontSize: 14, color: "#333", minHeight: 38, lineHeight: 1.5 }}>
        {value || "—"}
      </div>
    </div>
  );
...
function SectionHeader({ number, title }) {
  return (
    <div style={{ gridColumn: "1 / -1", display: "flex", alignItems: "center", gap: 10, padding: "10px 0 6px", borderBottom: "2px solid #e8f4fd", marginBottom: 4, marginTop: number > 1 ? 12 : 0 }}>
      <span style={{ background: "#1a6fa8", color: "#fff", borderRadius: "50%", width: 22, height: 22, display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 600, flexShrink: 0 }}>{number}</span>
...
function ReadOnlyForm({ data, formId, formVersion }) {
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 18px", background: "linear-gradient(135deg, #1e3a5f, #16324f)", borderRadius: 10, marginBottom: 20 }}>
        <div style={{ fontSize: 13, color: "#9fc7f0" }}>Form ID: <strong style={{ color: "#fff", fontFamily: "monospace", fontSize: 14 }}>{formId || "—"}</strong></div>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ fontSize: 11, color: "#5a8ab0" }}>Submitted: {fmtDt(data.submittedAt)}</div>
          <div style={{ width: 1, height: 16, background: "rgba(255,255,255,0.15)" }} />
          <div style={{ fontSize: 12, color: "#cfe6ff", background: "rgba(255,255,255,0.08)", borderRadius: 20, padding: "3px 12px", fontWeight: 500, border: "1px solid rgba(255,255,255,0.1)" }}>v{formVersion || "—"}</div>
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 20px" }}>
        ...
        <div style={{ gridColumn: "1 / -1" }}>
          ...
          <div style={{ padding: "10px 14px", background: "#e8f4fd", borderRadius: 6, border: "1px solid #b3d9f7", fontSize: 16, color: "#1a3c5e", fontWeight: 600 }}>RM {total}</div>
        </div>
...
function SignatureDialog({ open, onConfirm, onCancel, existingData }) {
  ...
  return createPortal(
    <div onClick={e => { if (e.target === e.currentTarget) onCancel(); }} style={{ position: "fixed", inset: 0, zIndex: 99999, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "#fff", borderRadius: 12, padding: 24, width: "100%", maxWidth: 500, boxShadow: "0 8px 40px rgba(0,0,0,0.25)" }}>
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 16, fontWeight: 600, color: "#111", marginBottom: 4 }}>Approver Signature</div>
          <div style={{ fontSize: 13, color: "#666" }}>Draw your signature below, then tap Confirm</div>
        </div>
        <div style={{ border: "1.5px solid #d0d0d0", borderRadius: 8, background: "#fafafa", position: "relative", overflow: "hidden" }}>
          <div style={{ position: "absolute", bottom: 36, left: 12, right: 12, borderBottom: "1px dashed #e0e0e0", pointerEvents: "none" }} />
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 16, gap: 8 }}>
          <button onClick={() => { padRef.current?.clear(); setIsEmpty(true); }} style={{ padding: "8px 16px", borderRadius: 6, border: "1px solid #ccc", background: "#fff", color: "#555", cursor: "pointer", fontSize: 13 }}>Clear</button>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={onCancel} style={{ padding: "8px 16px", borderRadius: 6, border: "1px solid #ccc", background: "#fff", color: "#555", cursor: "pointer", fontSize: 13 }}>Cancel</button>
            <button onClick={() => { if (!padRef.current?.isEmpty()) onConfirm(padRef.current.toDataURL()); }} disabled={isEmpty} style={{ padding: "8px 20px", borderRadius: 6, border: "none", background: isEmpty ? "#b0bec5" : "#1a6fa8", color: "#fff", cursor: isEmpty ? "not-allowed" : "pointer", fontSize: 13, fontWeight: 500 }}>Confirm</button>
          </div>
        </div>
      </div>
    </div>,
...
function SignatureTrigger({ value, onChange, submitting }) {
  ...
  return (
    <>
      ...
      <div onClick={() => setDialogOpen(true)} style={{ border: value ? "2px solid #1a6fa8" : "2px dashed #bbb", borderRadius: 8, background: value ? "#f0f5fa" : "#fafafa", minHeight: 100, maxWidth: 460, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", position: "relative", overflow: "hidden", userSelect: "none" }}>
        {value ? (
          <>
            <img src={value} alt="Signature" style={{ maxWidth: "90%", maxHeight: 80, display: "block", pointerEvents: "none" }} />
            <div style={{ position: "absolute", top: 8, right: 8, background: "#1a6fa8", color: "#fff", borderRadius: 4, padding: "3px 10px", fontSize: 11, fontWeight: 500 }}>Tap to edit</div>
            <button onClick={e => { e.stopPropagation(); onChange(null); }} style={{ position: "absolute", top: 8, left: 8, background: "rgba(255,255,255,0.9)", border: "1px solid #ddd", borderRadius: 4, padding: "3px 10px", fontSize: 11, cursor: "pointer", color: "#c0392b" }}>Remove</button>
          </>
        ) : (
          <div style={{ textAlign: "center", color: "#999", pointerEvents: "none" }}>
            <div style={{ fontSize: 24, marginBottom: 6 }}>✍️</div>
            <div style={{ fontSize: 14, fontWeight: 500, color: "#555" }}>Tap to sign</div>
            <div style={{ fontSize: 12, marginTop: 4, color: "#aaa" }}>Opens a signing dialog</div>
          </div>
        )}
      </div>
      {value
        ? <div style={{ marginTop: 8, fontSize: 12, color: "#2d6a3f", display: "flex", alignItems: "center", gap: 6 }}><span>✓</span> Signature ready — click Approve below to submit</div>
        : <div style={{ marginTop: 8, fontSize: 12, color: "#999" }}>Please draw your signature above before approving.</div>
      }
...
function ApprovalBox({ layer, totalLayers, email, signedAt, status, outcome, isMine, onApprove, onReject, submitting }) {
  ...
  const badgeText = signed ? (rejected ? "Rejected" : "Approved") : isMine ? "Awaiting your action" : "Pending";

  return (
    <div style={{ border: `1px solid ${colors.border}`, borderRadius: 10, padding: "20px 22px", marginBottom: 14, background: colors.bg, opacity: (!signed && !isMine) ? 0.6 : 1 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 28, height: 28, borderRadius: "50%", flexShrink: 0, background: signed ? (rejected ? "#e24b4a" : "#2d6a3f") : isMine ? "#1a6fa8" : "#ccc", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 600 }}>{layer}</div>
          <div style={{ fontWeight: 500, fontSize: 15, color: "#1a3c5e" }}>Layer {layer} of {totalLayers} — Approval</div>
        </div>
        <span style={{ fontSize: 11, fontWeight: 500, padding: "3px 12px", borderRadius: 20, background: colors.badge.bg, color: colors.badge.color, whiteSpace: "nowrap" }}>{badgeText}</span>
      </div>
      ...
      <div style={{ display: "flex", gap: 40, flexWrap: "wrap", marginBottom: 14 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 14, color: "#444" }}>{signedAt ? new Date(signedAt).toLocaleString("en-MY", { dateStyle: "long", timeStyle: "short" }) : "—"}</div>
        </div>
        {signed && outcome && (
          <div>
      ...
      {signed && !rejected && (
        <div>
          ...
          <div style={{ padding: "8px 12px", background: "#fff", borderRadius: 6, border: "1px solid #e0e0e0", display: "inline-block", fontSize: 13, color: "#aaa" }}>Signature on file</div>
        </div>
      )}
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 16, paddingTop: 16, borderTop: "1px solid #e0e0e0", flexWrap: "wrap" }}>
            <button
              ...
              style={{ flex: 1, minWidth: 140, padding: "11px 20px", borderRadius: 8, border: "none", background: sig ? "#1a6fa8" : "#e0e0e0", color: sig ? "#fff" : "#aaa", fontSize: 14, fontWeight: 500, cursor: (submitting || !sig) ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
            >
              ✓ Approve Application
            </button>
            <button
              onClick={onReject}
              disabled={submitting}
              style={{ padding: "11px 20px", borderRadius: 8, border: "1px solid #f5b7b1", background: "#fff", color: "#a93226", fontSize: 14, fontWeight: 500, cursor: submitting ? "not-allowed" : "pointer" }}
            >
              ✕ Reject Application
            </button>
          </div>
...
export default function ApprovePage() {
  ...
  const handleSwitch = useCallback(() => {
    // Force account picker by logging out of current account silently then redirecting to login
    instance.logoutRedirect({
      account: accounts[0],
      postLogoutRedirectUri: window.location.href,
      onRedirectNavigate: () => false, // don't navigate away from logout
    }).catch(() => {
      // Fallback: just redirect to login with prompt=select_account
  ...
  useEffect(() => {
    ...
    // Block users outside the tenant immediately — no PA call needed
    if (!isAllowedTenant(account)) {
  ...
  const handleConfirmApprove = useCallback(async () => {
    ...
    const doSubmit = async () => {
      ...
      try {
        ...
        setStatus("done"); retryFnRef.current = null;
      } catch (e) {
  ...
  if (status === "wrong_tenant") return (
    <WrongTenantScreen userEmail={userEmail} onLogout={handleLogout} onSwitch={handleSwitch} />
  );
  ...
  if (status === "error") return (
    <Screen icon="❌" title="Something Went Wrong" message={errorMsg} color="#a93226">
      <button onClick={() => { setStatus("idle"); window.location.reload(); }} style={{ background: "#1a6fa8", color: "#fff", border: "none", borderRadius: 8, padding: "10px 24px", fontSize: 14, cursor: "pointer" }}>Try again</button>
    </Screen>
  );

  if (status === "submit_error") return (
    <Screen icon="❌" title="Submission Failed" message={errorMsg} color="#a93226">
      <button onClick={() => retryFnRef.current?.()} style={{ background: "#1a6fa8", color: "#fff", border: "none", borderRadius: 8, padding: "10px 24px", fontSize: 14, cursor: "pointer" }}>Try again</button>
    </Screen>
  );
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "32px 20px" }}>
      ...
      {/* User badge — replaces old top banner */}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
        ...
        <span style={{ fontSize: 12, color: "#888" }}>({layers.filter(l => l.status === "Signed").length} of {total} completed)</span>
      </div>
