  ...
  const onComplete = useCallback(async (sender) => {
    ...
    try {
      const res = await fetch(process.env.REACT_APP_FLOW_URL, {
        ...
        body: JSON.stringify({ ...sender.data,startDate: toUTC(sender.data.startDate), endDate: toUTC(sender.data.endDate), formId: FORM_ID, formVersion: FORM_VERSION, submittedAt: new Date().toISOString(), baseUrl: window.location.origin }),
      });
